...
const seedMockDatabase = async () => {
  try {
    ...
    for (const data of mockData) {
      console.log(`Seeding parameter: ${data.parameter}...`);
      const newQuiz = new Quiz({
        parameter: data.parameter,
        questions: data.questions
      });
      await newQuiz.save();
      console.log(`Successfully seeded: ${data.parameter}`);
    }

    console.log("-----------------------------------------");
    console.log("PRO MOCK QUIZ SEEDING COMPLETE!");
    console.log("72 Professional Questions are now live.");
    console.log("-----------------------------------------");
    process.exit();
  } catch (error) {
